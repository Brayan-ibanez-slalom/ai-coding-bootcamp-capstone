...
The backend sends the side to move, FEN, difficulty ceiling, and complete legal UCI move list to Bedrock. It expects strict JSON:
...
The proposed move is validated with `chess.js` before returning SAN to the frontend. Retry one invalid/empty response. If Bedrock still fails, return the first legal `chess.js` move with `aiFallback: true` and explicit fallback advice. Return HTTP 503 only when no legal fallback exists.
